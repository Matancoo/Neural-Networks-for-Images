...
def generate_style_mix(
    ...
    # all_seeds.append(0) -- to get the style image
    

    # W --normal run on intermediary laten space W
    all_z = np.stack([np.random.RandomState(seed).randn(G.z_dim) for seed in all_seeds])
    all_w = G.mapping(torch.from_numpy(all_z).to(device), None)

    #appending the Van Gogh style image
    # all_z = np.concatenate((all_z,style_laten_vec), axis=0)
    # print(all_z.shape)
    # print(all_w.shape)
    style_w = torch.tensor(np.load('/content/drive/MyDrive/Github/output_pics/Style_im_latenCode/projected_w.npz')['w']).to(device)
    
    w_avg = G.mapping.w_avg
    all_w = w_avg + (all_w - w_avg) * truncation_psi
    
    # all_w = torch.cat((all_w,style_w)) #--- dont need only need to update dict
    # print(all_w.shape)
    w_dict = {seed: w for seed, w in zip(all_seeds, list(all_w))}
    # w_dict['style'] = style_w  #-- why this doesnt work??
